		...
		<div class="col-md-9" style="margin-left:30%">
			<div class="col-md-6">
			    ...
			    <!-- <div style='float:left;border:5px solid red;width:80%' id="summernote" >
			     -->
			    <textarea placeholder="댓글을 입력하세요" id="summernote" class="form-control" type=text size="250" name="gcomment"  ></textarea>
				<!-- </div>
				 -->
				<div style='float:left;border:5px solid green;width:30% '>
				<input id="submit" class="btn btn-info" value="확인" />
				</div>
				<!-- 
				<div style='float:left;border:5px solid blue;width:30% '>
				<input id="imgInput" type="file" class="btn btn-success" value="이미지"  />
				</div>
				 -->
				</div>
				</form>
...
var displayComments = function(data){	
	...
	$.each(data,function(index,comment){
		 ...
		 */
		commentsString+="<div style='margin-top: -15px;margin-left: 75px;width:300px;'><span style='margin-left:-25px;cursor:pointer' class='commentEdit' title='"+comment['GNO']+"'>"+comment['GCOMMENT']+'</span></div>';
		commentsString+="<span style='color:black;margin-left:50px;'>"+comment['GPOSTDATE']+"</span>";
		...
		$('#submit').click(function(){			
			if($('#submit').val()=='댓글수정'){			
				...
				$.ajax({
					...
					dataType:'text',
					type:'post',
					success:function(){showComments(); $('#submit').val('확인');$('#summernote').val('');
					$('#summernote').focus();}
				})
			 }; 	
			});		
